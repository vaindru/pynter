...
class TestDefectEntry(PynterTest):
    ...
    def get_entry(self):
        bulk_structure = get_object_from_json(Structure,
                                              self.get_testfile_path('Si-bulk_structure_3x3x3_supercell.json'))
    ...
    def test_relaxation_volume(self):
        relaxation_volume = self.entry.relaxation_volume(self.stress_bulk,
                                                         self.bulk_modulus)  # doesn't make physical sense with charged defect
