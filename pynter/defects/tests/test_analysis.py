...
class TestDefectsAnalysisComplexes(PynterTest):
    ...
    def test_defect_concentrations_fixed(self):
        ...
        desired_conc = DefectConcentrations([
            ...
            SingleDefConc(charge=-1.0, conc=2.53e+12, name=DefectComplexName.from_string("Sub_B_on_Si-Sub_P_on_Si"),
                          stable=True),
            SingleDefConc(charge=0.0, conc=2.79e+15, name=DefectComplexName.from_string("Sub_B_on_Si-Sub_P_on_Si"),
                          stable=True),
            SingleDefConc(charge=1.0, conc=1.21e+05, name=DefectComplexName.from_string("Sub_B_on_Si-Sub_P_on_Si"),
                          stable=True),
...
class TestDefectConcentrations(PynterTest):
    ...
    def test_elemental(self):
        self.assert_object_almost_equal(self.conc.elemental,
                                        {'Si': 9.504324340294853e-33, 'B': 1.4920566601530766e+16, 'P': 1e+17},
                                        rtol=100, atol=1e-02)
