...
class TestDataset(PynterTest):
    ...
    def test_select_jobs(self):
        ...
        DatasetTest().assert_jobs_equal(ds.select_jobs(groups=['q1'], common_node='1-PBE-SCF', mode='or'),
                                        [ds[i] for i in (0, 2, 4, 5)])
    ...
    def test_sort_jobs(self):
        ds = self.ds
        DatasetTest().assert_jobs_equal(ds.sort_jobs(features='charge', reset=False),
                                        ds.sort_jobs(features='name', reset=False))
