...
def setup_inputs_vasp(parser):
    ...
    parser.add_argument('-cs', '--charge-states', action='append',
                        help='Charged states. Provide a list of integer charge states', required=False,
                        type=int, default=None, metavar='', dest='charge_states')

    parser.add_argument('-dp', '--dielectric-properties', action='store_true',
                        help='Electronic and ionic parts of the dielectric tensor', required=False,
                        default=False, dest='dielectric_properties')

    parser.add_argument('-ec', '--encut-convergence', action='store_true', help='Energy cutoff convergence',
                        required=False,
                        default=False, dest='encut_convergence')

    parser.add_argument('-fc', '--fractional-charge', action='store_true', help='Fractional charge linearity',
                        required=False,
    ...
    parser.add_argument('-hse', '--hse-relaxation', action='store_true',
                        help='5 steps atomic and volume relaxation with HSE', required=False,
                        default=False, dest='hse_relaxation')

    parser.add_argument('-pbe', '--pbe-relaxation', action='store_true',
                        help='3 steps atomic and volume relaxation with PBE', required=False,
                        default=False, dest='pbe_relaxation')

    parser.add_argument('-pbe-es', '--pbe-electronic-structure', action='store_true',
                        help='3 steps electronic structure calculations with PBE', required=False,
                        default=False, dest='pbe_electronic_structure')

    parser.add_argument('-is', '--inputsets', help='Name of a set of inputs from the InputSets class', required=False,
                        type=str,
...
def parse_common_args(parser):
    parser.add_argument('-id', '--materials-project-id',
                        help='Materials project ID containing the structure, in case the POSCAR is not provided (default: %(default)s)',
                        required=False, type=str, default=None, metavar='', dest='mp_id')

    parser.add_argument('-n', '--name', help='Name of the folder containing input files (default: %(default)s)',
...
def parse_vasp_args(parser):
    parser.add_argument('-in', '--incar',
                        help='INCAR file with general settings, if not provided default settings are used',
                        required=False, type=str, default=None, metavar='',
                        dest='incar')

    parser.add_argument('-kp', '--kpoints', help='KPOINTS file, if not provided default settings are used',
                        required=False, type=str, default=None, metavar='',
                        dest='kpoints')

    parser.add_argument('-pos', '--poscar', help='POSCAR file (default: %(default)s)', required=False, type=str,
                        default=None, metavar='',
                        dest='poscar')

    parser.add_argument('-pot', '--potcar', help='POTCAR file (default: %(default)s)', required=False, type=str,
                        default=None, metavar='',
                        dest='potcar')
