...
def setup_phase_diagram(subparsers):
    parser_sub = subparsers.add_parser('phase-diagram',
                                       help='Generate phase diagrams with pymatgen from the Materials Project database')

    parser_sub.add_argument('-e', '--elements', action='append',
                            help='Components of the phase diagram, provide as "el1-el2" or -e el1 -e el2',
                            type=str, metavar='', dest='elements')
    parser_sub.add_argument('-c', '--chempots', help='Print boundary chempots of target phase', required=False,
                            default=False, type=str, metavar='',
                            dest='composition_chempots')
    parser_sub.add_argument('-sc', '--save-chempots', action='store_true',
                            help='Save chempots in Reservoirs object as json', required=False, default=False,
                            dest='savechempots')
    parser_sub.add_argument('-p', '--plot-PD', action='store_true', help='Plot phase diagram', required=False,
                            default=False, dest='plot_pd')
    parser_sub.add_argument('-ps', '--plot-stability', action='append',
                            help='Plot stability diagram, provide elements to be on axis as "el1-el2" or -ps el1 -ps el2'
                            , required=False, type=str, dest='elements_stability')
    parser_sub.add_argument('-j', '--json', action='store_true', help='Save PhaseDiagram as json', required=False,
                            default=False, dest='savejson')
    parser_sub.add_argument('-s', '--savefig', action='store_true', help='Save matplotlib figure as pdf',
                            required=False, default=False, dest='savefig')
...
def run_phase_diagram(args):
    ...
    if args.elements_stability:
        elements_stability = args.elements_stability[0].split('-') if len(
            args.elements_stability) == 1 else args.elements_stability
