...
class InputSets:
    ...
    def __init__(self, path=None, vaspinput=None, structure=None, incar_settings=None, kpoints=None, potcar=None,
        ...
        if structure:
            self.incar_settings = incar_settings if incar_settings else DefaultInputs(
                self.structure).get_incar_default()
            ...
            raise ValueError(
                'You need to provide Structure, either as Poscar object in VaspInput or in "structure" arg')
    ...
    def get_vaspjob(self, setname='', pathname=''):
        ...
        vaspjob = VaspJob(path=jobpath, inputs=vaspinput, job_settings=job_settings,
                          job_script_filename=job_script_filename, name=jobname)
...
class Schemes(InputSets):
    ...
    def dielectric_properties_electronic(self, scheme_name='eps-electronic'):
        ...
        nelect = sum([val[el] * self.structure.composition.as_dict()[el] for el in
                      self.structure.composition.as_dict()])  # number of electrons
    ...
    def fractional_charge_linearity(self, nelect=None):
        ...
        if not nelect:
            nelect = sum(
                [val[el] * self.structure.composition.as_dict()[el] for el in self.structure.composition.as_dict()])
...
class AdvancedSchemes(Schemes):
    ...
    def defects_pbe_relaxation(self, defects_with_charges, automation=False, locpot=False, rel_scheme='default'):
        ...
        if automation:
            self.job_settings[
                'add_automation'] = 'pynter automation vasp --contcar --chgcar --wavecar --error-check --check-kpoints'
        ...
        for df, charge_states in defects_with_charges:
            ...
            schemes_q = Schemes(path=path, structure=df.defect_structure, incar_settings=self.incar_settings,
                                job_settings=self.job_settings, name=self.name + '_' + defect_name,
    ...
    def substitutions_pbe_relaxation(self, elements_to_replace_with_charges, supercell_size=None, automation=False,
                                     locpot=False, rel_scheme='default'):
        """
        ...
        jobs = self.defects_pbe_relaxation(defects_with_charges, automation=automation, locpot=locpot,
                                           rel_scheme=rel_scheme)
        return jobs

    def vacancies_pbe_relaxation(self, elements_with_charges, supercell_size=None, automation=False, locpot=False,
                                 rel_scheme='default'):
        """
...
class NEBSchemes:
    ...
    def __init__(self, path, structures, incar_settings=None, kpoints=None, potcar=None, job_settings=None, name=None,
        ...
        self.incar_settings = incar_settings if incar_settings else DefaultInputs(
            self.structures[0]).get_incar_default()
