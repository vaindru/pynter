...
class CommandHandler(Automation):
    ...
    def parser_common_args(self):
        ...
        parser.add_argument('-j', '--job-script', help='Job script filename, default is "job.sh"', required=False,
                            default=self.job_script_filename, type=str, metavar='', dest='job_script_filename')
        parser.add_argument('-s', '--status', help='Write exit status to file, default is "exit_status.txt"',
                            required=False, default=self.status_filename, type=str, metavar='', dest='status_filename')
        parser.add_argument('-e', '--error-check', action='store_true', help='Perform error checking. Default is False',
                            required=False, default=False, dest='error_check')
    ...
    def vasp_args(self):
        ...
        # part for VASP
        parser.add_argument('-c', '--contcar', action='store_true', help='Copy CONTCAR to POSCAR of next step',
                            required=False, default=False, dest='contcar')
        parser.add_argument('-W', '--wavecar', action='store_true', help='Copy WAVECAR to next step', required=False,
                            default=False, dest='wavecar')
        parser.add_argument('-C', '--chgcar', action='store_true', help='Copy CHGCAR to next step', required=False,
                            default=False, dest='chgcar')
        parser.add_argument('-K', '--check-kpoints', action='store_true',
